...
// leetcode - 188
// striver used 2d dp...check it out
class Solution {
public:
    ...
    int memoization(int ind,bool buy,vector<int> &prices,int cap,vector<vector<vector<int>>> &dp)
    {
        if(cap == 0)
            return 0;
        if(ind == prices.size())
    ...
    int maxProfit(int k, vector<int>& prices) {
        ...
        return memoization(0,1,prices,k,dp);*/
