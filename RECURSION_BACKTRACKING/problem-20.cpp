...
public:
    ...
    void compute(int ind,int n,vector<string> &s,vector<vector<string>> &ans)
    {
        if(ind == n )
        {
            ans.push_back(s);
    ...
    void compute1(int ind,int n,vector<string> &s,vector<vector<string>> &ans,vector<int> &row,vector<int> &ud,vector<int> &ld)
    {
        if(ind == n )
        {
            ans.push_back(s);
    ...
    vector<vector<string>> solveNQueens(int n) {
        ...
        // optimal -> recursion, hashing, for loop tc-o(n!) sc-o(n^2+5n)
        vector<vector<string>> ans;
        vector<string> s;
        for(int i=0;i<n;i++)
        {
